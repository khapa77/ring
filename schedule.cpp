...
bool editScheduleEntry(uint32_t id, uint8_t hour, uint8_t minute, const String& description) {
    if (hour > 23 || minute > 59) {
        return false;
    }
    ...
    for (uint8_t i = 0; i < scheduleCount; i++) {
        if (scheduleEntries[i].id == id) {
            ...
            Serial.printf("Edited schedule ID: %u to %02d:%02d - %s\n", 
                        id, hour, minute, description.c_str());
            return true;
        }
    }
    return false;
